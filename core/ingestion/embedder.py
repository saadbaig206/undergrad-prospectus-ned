...
class ONNXEmbeddings(Embeddings):
    ...
    def _embed(self, texts: list[str], mode: str = "query") -> list[list[float]]:
        if self.use_api:
        ...
        # 3. Asymmetric Model Pooling (Model Specific)
        if mode == "query":
            # Mean Pooling for Queries
            input_mask_expanded = np.expand_dims(attention_mask, axis=-1)
            sum_embeddings = np.sum(last_hidden_state * input_mask_expanded, axis=1)
            sum_mask = np.clip(np.sum(input_mask_expanded, axis=1), a_min=1e-9, a_max=None)
            target_embeddings = sum_embeddings / sum_mask
        else:
            # [CLS] pooling for Indexed Documents (crucial for NoInstruct-small)
            target_embeddings = last_hidden_state[:, 0, :]
        
        # 4. Normalization
        norms = np.linalg.norm(target_embeddings, ord=2, axis=1, keepdims=True)
        norms = np.clip(norms, a_min=1e-9, a_max=None)
        normalized_embeddings = target_embeddings / norms
    ...
    def embed_documents(self, texts: list[str]) -> list[list[float]]:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            results.extend(self._embed(batch, mode="document"))
        return results
